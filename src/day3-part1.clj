...
(defn count-dupes [rucksacks]
  (let [dupes (map #(clojure.set/intersection (set (first %)) (set (second %))) rucksacks)
        priorities (map priority-map (map first dupes))]
    priorities))

(def result (apply + (count-dupes rucksacks)))
